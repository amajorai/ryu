...
const wordCount = defineTool<{ text: string }, number>({
  ...
  run(input) {
    return Promise.resolve(input.text.trim().split(/\s+/).filter(Boolean).length);
  },
});

export const summarizeAndRate = defineWorkflow<{ text: string }, { words: number; summary: string }>({
...
if (import.meta.main) {
  ...
  const out = await summarizeAndRate.run({ text }, { gateway: undefined as never });
