...
export class MarkdownJoiner {
	...
	processText(text: string): string {
		...
		for (const char of text) {
			if (
				...
				output += this.processStreamingChar(char);
			} else if (this.isBuffering) {
				output += this.processBufferedChar(char);
			} else if (
				char === "*" ||
				char === "<" ||
				char === "`" ||
	...
	private processStreamingChar(char: string): string {
		this.buffer += char;
		let output = "";
	...
	private processBufferedChar(char: string): string {
		this.buffer += char;

		if (this.startNestedBufferIfReady()) {
			return "";
		}

		if (this.isCompleteBufferedConstruct() || this.isFalsePositive(char)) {
	...
	private startNestedBufferIfReady(): boolean {
	...
	private isCompleteBufferedConstruct(): boolean {
		return (
			this.isCompleteBold() ||
...
function delay(delayInMs?: number | null): Promise<void> {
