...
Ryu is undergoing a systematic decomposition from a monolithic Core (~195k LoC) into ~40
...
1. **Own package** — code has moved to its own crate (`crates/ryu-*`) or app directory (`apps-store/<app>/`)
...
|---|---|---|---|
| **W0** | Root virtual workspace, close raw-route gaps | ~2-3 days | In progress |
| **W1** | Tracer extractions: `crates/ryu-quests`, `ryu-recipes`, `ryu-clips` | ~3-5 days | In progress |
| **W1.5** | Layout normalization to `apps-store/<app>/{plugin.json, backend/, sidecar/, ui/, tests/}` | ~1 day | Done |
| **W2** | L0 primitives: `ryu-storage`, `ryu-crypto`, `ryu-engines`, `ryu-downloads`, `ryu-vault` | ~2-3 weeks | Not started |
| **W3** | L1 primitives: 13 packages (rag, tts, stt, vad, image, collab, realtime, workspace, sandbox, webhook-ingress, mesh, usage-metering, email-send) | ~2-3 weeks | Not started |
| **W4** | L2 features: 14 packages (memory, spaces, retrieval, search, knowledge, tool-registry, tool-exec, mcp-catalog, model-catalog, composio, identity, activity, tracing, sync) | ~2-3 weeks | Not started |
| **W5** | L3 apps to sidecars: 15+ app packages (meetings, monitors, approvals, learning, healing, research, teams, dashboards, skills, hardware, workflows, agents-catalog, predict, voice, finetune) | ~2-3 weeks | Not started |
| **W6** | Gateway: open `ProviderKind`, config-driven `set_active`, invert firewall/DLP + router + smart-router + passthrough to traits, stage crates | ~2 weeks | Not started |
...
| **W8** | Platform: extension host runtime, versioned host API, dev loop, activation events, grant consent, shadcn-style UI registry, file-based definition distribution | ~1-2 months | Not started |
| **W9** | Browser app: `apps-store/browser/` with Electron sidecar + real Chromium + CDP, `browser.control` capability | ~2-3 weeks | Not started |
...
Extract shared types into a pure-data crate (`crates/ryu-kernel-contracts`). Both Core and
apps depend on the contracts crate, so neither can drift from the wire format.

### 5. Verify-every-wave

Each wave ships with an integration test suite (`crates/ryu-integration-tests`) that proves
...
| Notifications | 4 dispatch shapes — not a single capability | ~2k |
| ~30 UTIL modules | Small helpers not worth extracting | ~10k |
| **Total** | | **~37k** |

The estimated end-state kernel is **~40-50k LoC** (down from ~195k today).
...
The 5-agent deep audit found:

- **Physical extraction: ~5%** — only mail has been physically moved to its own package
- **Capability graph: 3 nodes / 2 edges** out of ~40 mapped — the graph is mostly aspirational
- **Mechanisms are strong** — the broker, contracts crate, and sidecar loader are built and working
- **Migration is weak** — most code is still in-crate; the extraction has not started in earnest

This is honest: the decomposition infrastructure is built, but the extraction itself is early.
