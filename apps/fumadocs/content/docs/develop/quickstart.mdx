...
have a valid `plugin.json` and an agent whose every model call routes through the Ryu gateway.

You write plugins with `@ryu/sdk` (the Ryu developer SDK) and scaffold them with
...
[the plugin.json manifest](/docs/develop/sdk/plugin-api).
...
| File | What it is |
|---|---|
| `plugin.json` | The plugin manifest, validated against `PluginManifestSchema` so the desktop plugin store can install it |
| `src/agent.ts` | A starter Runnable that streams one turn through the gateway-mandatory model client |
| `package.json` | A minimal project with a `@ryu/sdk` dependency plus `dev` and `pack` scripts |
...
`plugin.json`, validates the result against `PluginManifestSchema`, and writes `package.json`. On
...
The generated `plugin.json` looks like this (your name is stamped into the `id` and `name`):
...
This pulls `@ryu/sdk`, which gives you the gateway-mandatory model client and the
`defineAgent` / `defineWorkflow` / `defineTool` / `defineSkill` Runnable factories.

## Step 3: Read the starter agent

Open `src/agent.ts`. It defines a model with `defineModel` from `@ryu/sdk/model` and streams one
turn:
...
This calls `bunx ryu pack .`. The `ryu` CLI (`packages/sdk/src/cli.ts`) validates `plugin.json`
...
|---|---|
| `bunx ryu pack <dir>` | Validate `plugin.json` and emit `dist/plugin.bundle.json` |
...
Because `plugin.json` validates against the schema the desktop plugin store reads, your bundle
