...
tags: ["plugin", "manifest", "plugin.json", "runnable", "install", "extension", "packaging"]
---
...
not yet type). The example below is taken from the built-in `sample.plugin.json` fixture.
...
<Callout type="warn">
  ...
  does not type — write the `plugin.json` directly rather than relying on the SDK type for those.
</Callout>
