...
impl ReplicateProvider {
    ...
    async fn create_prediction(&self, model: &str, input: Value) -> Result<Value, GatewayError> {
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| GatewayError::ProviderError(format!("replicate request failed: {e}")))?;
        parse_prediction(resp, "replicate create").await
    }

    /// Fetch a prediction's current state by id.
    async fn get_prediction(&self, id: &str) -> Result<Value, GatewayError> {
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| GatewayError::ProviderError(format!("replicate poll failed: {e}")))?;
    ...
    async fn run_inline(&self, model: &str, body: &Value) -> Result<Value, GatewayError> {
        ...
        if replicate_status(&current) == JobStatus::Failed {
            ...
            return Err(GatewayError::ProviderError(format!("replicate: {err}")));
        }
...
async fn parse_prediction(resp: reqwest::Response, ctx: &str) -> Result<Value, GatewayError> {
