...
// Spaces tab - parity with apps/cli's Spaces tab
// (apps/cli/src/{app.rs,api.rs,ui.rs,main.rs}):
//   - data: GET /api/spaces (list), GET /api/spaces/:id/documents (per selected
//     space, lazily + cached by id), GET /api/conversations (history pane)
//   - layout: header " Spaces" | left column [spaces list (top) + documents for
//     the selected space (bottom)] ~60% · right column conversation history ~40%
//   - spaces rows: name + muted " (N docs)" count, selected highlighted
//   - documents pane: states none/loading/empty, else title + muted chunk count
//   - history pane: title (or "untitled") + muted message count + date, windowed
//     by a scroll offset
//   - keys: ↑/k ↓/j select space (reloads that space's documents), r refresh all
//     (and drops the document cache), PageUp/PageDown scroll the history pane
//   - read-only: spaces are created/edited in the desktop app (no data-grid here),
//     matching apps/cli
//
// No value is hardcoded - everything comes from Core. Keyboard is owned here and
// gated on `active` (and suppressed while a text input elsewhere owns the
// keyboard). termcn Card/Badge handle presentation.
...
// Core's GET /api/conversations wire shape (snake_case). Mirrors apps/cli's
// ConversationSummary; the response is `{ conversations: [...] }` or a bare array.
...
// it through the shared HTTP primitive (parity with apps/cli api::fetch_conversations).
...
export function SpacesTab({ active }: TabProps) {
	...
	// Load the spaces list and conversation history together (parity with apps/cli
	// refresh_spaces_data). The spaces fetch failing surfaces an error view; the
	// history fetch failing only flashes a toast so the tab stays usable.
	...
	// changes, mirroring apps/cli refresh_selected_space_docs. Already-cached
	// spaces are skipped so reselecting is instant.
	useEffect(() => {
