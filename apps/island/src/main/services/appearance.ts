...
/** Timeout for the one-shot appearance read. */
...
export async function getAppearanceRaw(): Promise<string | null> {
	...
	try {
		const resp = await fetch(
			`${coreBaseUrl}/api/preferences/${APPEARANCE_PREF_KEY}`,
			{
				method: "GET",
				headers: coreHeaders(),
...
/** Extract the appearance value from one SSE `data:` payload, or `null`. */
function appearanceValueFromFrame(payload: string): string | null {
	...
	try {
		...
		if (ev.key === APPEARANCE_PREF_KEY && typeof ev.value === "string") {
			return ev.value;
		}
	} catch {
...
function dispatchLines(
	...
	while (newlineIndex !== -1) {
		...
		if (line.startsWith("data:")) {
			const value = appearanceValueFromFrame(line.slice("data:".length));
			if (value !== null) {
				onValue(value);
			}
		}
...
async function readAppearanceStream(
...
/**
 * Subscribe to live appearance changes via Core's `/api/preferences/stream` SSE
	...
	const connect = async (): Promise<void> => {
		...
		try {
			...
			if (resp.ok && resp.body) {
				await readAppearanceStream(resp.body, onValue, () => stopped);
			}
		} catch {
