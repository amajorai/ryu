//! Managed child process for the Shadow sidecar.
...
fn shadow_dir() -> PathBuf {
    crate::paths::ryu_dir().join("shadow")
}

fn pid_path() -> PathBuf {
    shadow_dir().join("shadow.pid")
}
...
impl ShadowProcess {
    ...
    /// Spawn `shadow start` and begin forwarding its stdio to tracing.
    pub async fn start(&mut self) -> Result<()> {
        ...
        if let Some(stdout) = child.stdout.take() {
            let handle = tokio::spawn(async move {
                ...
                while let Ok(Some(line)) = lines.next_line().await {
                    tracing::info!(target: "shadow", "{line}");
                }
            });
        ...
        if let Some(stderr) = child.stderr.take() {
            let handle = tokio::spawn(async move {
                ...
                while let Ok(Some(line)) = lines.next_line().await {
                    tracing::warn!(target: "shadow", "{line}");
                }
            });
            self.log_tasks.push(handle);
        }
    ...
    pub async fn stop(&mut self) -> Result<()> {
        ...
        match tokio::time::timeout(Duration::from_secs(5), child.wait()).await {
            Ok(_) => {}
            Err(_) => {
                tracing::warn!("shadow did not exit within 5 s — sending SIGKILL");
                let _ = child.kill().await;
