//! SGLang installer — checks for Python ≥ 3.9 and installs sglang via pip.
//!
//! Mirrors the vLLM installer: SGLang is also a Python package launched as an
//! OpenAI-compatible server. We install the base `sglang` distribution; GPU
//! extras (`sglang[all]`) are left to the user since they pull large CUDA deps.
...
pub async fn python_cmd() -> Result<String> {
    for candidate in ["python3", "python"] {
            ...
            .await
        {
            if output.status.success() {
                ...
                bail!(
                    "Python {version_str} found but SGLang requires Python ≥ 3.9. \
    ...
    bail!(
        "Python not found. SGLang requires Python ≥ 3.9. \
...
pub async fn ensure_installed() -> Result<()> {
    ...
    if store.versions.contains_key("sglang") {
        tracing::info!("sglang already installed — skipping");
        return Ok(());
    }

    let python = python_cmd()
        .await
        .context("locating Python interpreter for SGLang")?;

    tracing::info!("installing sglang via pip ({python} -m pip install sglang)");
    ...
    if !status.success() {
        bail!("`{python} -m pip install sglang` failed with {status}");
    }
    ...
    VersionStore::set_version_persisted("sglang", &version).context("writing versions.json")?;

    tracing::info!("sglang {version} installed");
    Ok(())
}
