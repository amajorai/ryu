//! Durable-execution host shim: Core's in-process [`FallbackEngine`] binding of
//! the `ryu-durable` [`DurableEngine`] seam.
//!
//! # Design
//!
//! The `DurableEngine` trait and the atomically-durable checkpoint store live in
//! the extracted `ryu-durable` crate (the durable-execution primitive). This
//! module is the Core-side host wiring: it binds that seam to the concrete
//! workflow model (`Workflow` / `WorkflowRun`) and the petgraph executor, which
//! stay Core-side because they *are* the workflow-app. Today there is one engine:
//!
//! - **`FallbackEngine`** — the in-process topological executor backed by
//!   `store::save_run` / `store::load_run` (which persist through the crate's
//!   `FileCheckpointStore`). It runs on every platform, requires no extra sidecar,
//!   and is durable at the node-checkpoint level: run state is rewritten to disk
//!   after every node (and after every loop iteration), so a run survives a Core
//!   restart and resumes from the last completed node.
//!
...
//! semantics directly. The seam (the crate's [`DurableEngine`] trait +
//! [`select_engine`]) is kept so a future durable backend (Temporal / Restate /
//! DBOS sidecar) can slot in with no server-handler churn.
//!
//! # Core-vs-Gateway
//!
...
use ryu_durable::{BoxFuture, DurableEngine};
...
/// that binds the `ryu-durable` [`DurableEngine`] seam to Core's `Workflow` /
/// `WorkflowRun` model.
///
...
impl DurableEngine<Workflow, WorkflowRun> for FallbackEngine {
    fn execute<'a>(
        &'a self,
        workflow: &'a Workflow,
        input: HashMap<String, String>,
        run_id: String,
    ) -> BoxFuture<'a, Result<WorkflowRun, String>> {
        Box::pin(executor::run_workflow(workflow, input, run_id))
    }

    fn checkpoint<'a>(&'a self, run: &'a WorkflowRun) -> BoxFuture<'a, Result<(), String>> {
        Box::pin(
    ...
    fn resume<'a>(
        ...
        run_id: &'a str,
        workflow_id: &'a str,
    ) -> BoxFuture<'a, Option<WorkflowRun>> {
        Box::pin(async move {
...
pub fn select_engine() -> Box<dyn DurableEngine<Workflow, WorkflowRun>> {
