//! Plugin-owned key/value storage.
//!
//! Each plugin gets an isolated, namespaced KV space exposed **only** through the
//! plugin-host `storage` capability (see [`crate::plugin_host`]), gated by the
//! `storage:kv` grant. This is where a plugin keeps durable state instead of Core
//! growing bespoke columns for it — e.g. the goal plugin's per-conversation
//! completion condition + turn count live here (key = conversation id), not on
//! the `conversations` table.
//!
//! Placement (Core vs Gateway): this stores *what a plugin is tracking* — it
//! decides what runs, not what is allowed — so it is Core. Rows are namespaced by
//! `(plugin_id, namespace, key)` so one plugin can never read another's state.
