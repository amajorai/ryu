...
use super::{DownloadEvent, DownloadKind, DownloadSpec, DownloadState, DownloadTask};
use crate::sidecar::download_manager::{build_http_client, ryu_dir, VersionStore};
...
async fn fast_path(spec: &DownloadSpec) -> Option<PathBuf> {
    ...
    let expected = spec.sha256.clone().filter(|s| !s.is_empty()).or_else(|| {
        spec.version_record.as_ref().and_then(|v| {
            VersionStore::load()
                .installed_checksum(&v.store_key)
                .map(str::to_string)
        })
    })?;
...
async fn stream_once(
    ...
    let mut req = inner.client.get(&spec.url);
    if crate::hf_auth::is_hf_url(&spec.url) {
...
async fn finalize(
    ...
    if let Some(rec) = &spec.version_record {
        let _ = VersionStore::record_persisted(&rec.store_key, &rec.version, &actual);
    }
...
mod tests {
    use super::*;
    use crate::downloads::DownloadKind;
