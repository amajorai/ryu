...
//! [`crate::experience`] owns storage and [`crate::server::learning`] is the HTTP
//! surface. Nothing here touches the chat hot path: the experience buffer is
//! built by **sweeping** the persisted conversation store at cycle time, so a
...
//!   *original base* model (never a previous merge), the input to the existing
//!   Unsloth fine-tune path.
...
/// the monitor/quest/identity-health engines' `global_engine()` singletons.
...
// ---------------------------------------------------------------------------
// Resolvers (pref -> env -> default), mirroring resolve_auto_recall_* in server
// ---------------------------------------------------------------------------
...
pub async fn resolve_enabled(state: &ServerState) -> bool {
    if let Some(v) = pref(state, LEARNING_ENABLED_PREF).await {
        return truthy(&v);
    }
...
pub async fn resolve_excluded(state: &ServerState, conversation_id: &str) -> bool {
    let key = format!("{LEARNING_EXCLUDE_PREFIX}{conversation_id}");
    pref(state, &key).await.map(|v| truthy(&v)).unwrap_or(false)
}
...
async fn conversation_excluded(
    ...
    let v = resolve_excluded(state, conversation_id).await;
    cache.insert(conversation_id.to_string(), v);
    v
}

pub async fn resolve_min_reward(state: &ServerState) -> f64 {
    pref(state, LEARNING_MIN_REWARD_PREF)
        .await
...
pub async fn resolve_in_sleep_window(state: &ServerState) -> bool {
    let start = pref(state, LEARNING_SLEEP_START_PREF)
        .await
        .and_then(|v| v.parse::<u32>().ok())
        .filter(|h| *h < 24);
    let end = pref(state, LEARNING_SLEEP_END_PREF)
        .await
        .and_then(|v| v.parse::<u32>().ok())
        .filter(|h| *h <= 24);
...
pub async fn scheduled_cycle_due(state: &ServerState) -> bool {
    let last = pref(state, LEARNING_LAST_CYCLE_PREF)
        .await
        .and_then(|v| v.parse::<i64>().ok());
    let Some(last) = last else {
        return true;
    };
    let gap_hours = pref(state, LEARNING_MIN_GAP_HOURS_PREF)
        .await
        .and_then(|v| v.parse::<i64>().ok())
        .filter(|h| *h > 0)
...
pub async fn mark_cycle_ran(state: &ServerState) {
    let now = chrono::Utc::now().timestamp().to_string();
    let _ = state.preferences.set(LEARNING_LAST_CYCLE_PREF, &now).await;
}
...
pub async fn resolve_skill_generation(state: &ServerState) -> i64 {
    pref(state, LEARNING_SKILL_GENERATION_PREF)
        .await
        .and_then(|v| v.parse::<i64>().ok())
        .unwrap_or(0)
}

pub async fn resolve_base_model(state: &ServerState) -> Option<String> {
    pref(state, LEARNING_BASE_MODEL_PREF).await
}
...
pub async fn resolve_prm(state: &ServerState) -> ModelSource {
    let model = pref(state, LEARNING_PRM_MODEL_PREF)
        .await
...
pub async fn resolve_synth(state: &ServerState) -> ModelSource {
    let model = pref(state, LEARNING_SYNTH_MODEL_PREF)
        .await
...
pub async fn resolve_config(state: &ServerState) -> LearningConfig {
    let prm = resolve_prm(state).await;
    ...
    LearningConfig {
        ...
        min_reward: resolve_min_reward(state).await,
        base_model: resolve_base_model(state).await,
        skill_generation: resolve_skill_generation(state).await,
    }
}

// ---------------------------------------------------------------------------
// Model call: Gateway (call_side_model) or BYO OpenAI-compatible endpoint
// ---------------------------------------------------------------------------
...
async fn run_model(
    ...
    let Some(url) = src.url.as_deref() else {
        // Gateway path: reuse the shared side-model primitive (handles auth).
        return crate::server::call_side_model(state, &src.model, &src.effort, system, user).await;
    };
    ...
    let mut req = state.client.post(&endpoint).json(&body);
...
pub async fn sweep_into_buffer(state: &ServerState) -> Result<usize> {
    ...
    for conv in conversations {
        ...
        if resolve_excluded(state, &conv.id).await {
            continue;
        }
        let messages = match state.conversations.get_messages(&conv.id).await {
            Ok(m) => m,
            Err(e) => {
...
pub fn parse_reward(text: &str) -> Option<f64> {
    ...
    let mut flush = |num: &mut String, had_dot: &mut bool, tokens: &mut Vec<(f64, bool)>| {
        if !num.is_empty() {
...
pub async fn score_buffer(state: &ServerState, limit: usize) -> Result<usize> {
    if !resolve_enabled(state).await {
        return Ok(0);
    }
    let src = resolve_prm(state).await;
    let pending = state
        .experience
        .list_unscored(limit)
        .await
    ...
    for exp in pending {
        ...
        if conversation_excluded(state, &exp.conversation_id, &mut exclude_cache).await {
            continue;
        }
        let user = format!(
            "User message:\n{}\n\nAssistant reply:\n{}",
...
fn transcript(messages: &[crate::server::conversations::StoredMessage]) -> String {
...
/// Consent-gated: runs only when the global opt-in is on OR `force` is set by a
/// deliberate per-conversation user action ("make a skill from this chat"). With
/// learning off and `force` false this is a no-op — conversation content is never
...
pub async fn synthesize_skill(
    ...
    let src = resolve_synth(state).await;
    let answer = run_model(state, &src, SYNTH_SYSTEM, &transcript(&messages))
        .await
    ...
    let gen = resolve_skill_generation(state).await + 1;
    let _ = state
        .preferences
        .set(LEARNING_SKILL_GENERATION_PREF, &gen.to_string())
        .await;
...
async fn write_skill(slug: &str, contents: &str) -> Result<()> {
    let dir = crate::skills::SkillRegistry::skills_dir().join(slug);
...
/// (training needs a GPU and is the opt-in heavy step). `execute` is reserved for
/// wiring the existing `/api/finetune/start` path; left undispatched here so the
/// scaffold is safe to call anywhere.
pub async fn run_cycle(state: &ServerState, execute: bool) -> Result<CyclePlan> {
    if !resolve_enabled(state).await {
        return Ok(CyclePlan {
            base_model: None,
            swept: 0,
    ...
    let swept = sweep_into_buffer(state).await?;
    let scored = score_buffer(state, 256).await?;
    let min_reward = resolve_min_reward(state).await;
    let base_model = resolve_base_model(state).await;
    ...
    for r in candidate_rows {
        if !conversation_excluded(state, &r.conversation_id, &mut exclude_cache).await {
            rows.push(r);
        }
    }
    ...
    if !rows.is_empty() {
        let dir = crate::paths::ryu_dir().join("learning");
    ...
    let mut error = None;
    let note = if execute {
        let d = dispatch_cycle(state, &base_model, &rows, &dataset_path, &write_err).await;
...
/// the shared finetune dispatch path, which GPU-gates and records the job. Training
...
async fn dispatch_cycle(
    ...
    match crate::server::finetune::dispatch(state, body).await {
        Ok(resp) => {
            out.dispatched = true;
            out.job_id = resp
                .get("job_id")
        ...
        Err((code, err)) => {
            ...
            fail(&mut out, format!("dispatch failed ({code}): {msg}"));
        }
    }
    out
}
...
async fn resolve_train_target(state: &ServerState) -> String {
    pref(state, "learning.train-target")
        .await
        .filter(|t| t == "remote")
...
async fn resolve_remote(state: &ServerState) -> Option<Value> {
    let url = pref(state, "learning.remote-url").await?;
    let mut remote = json!({ "url": url });
    if let Some(token) = pref(state, "learning.remote-token").await {
...
mod tests {
    ...
    fn build_skill_md_round_trips_through_parser() {
        ...
        let parsed = crate::skills::parse_skill_md("learned-x", &md).expect("valid skill");
