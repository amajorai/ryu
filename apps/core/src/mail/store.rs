...
//!
//! Mirrors [`crate::monitors::store`]: an `Arc<Mutex<Connection>>` with WAL and a
//! broadcast channel that fans freshly-stored messages out to SSE subscribers.
//! Attachment bytes live on the filesystem under `~/.ryu/mail-blobs/`, keyed by
//! sha256, so the DB stays small and blobs survive a data-folder relocation
//! (relative paths).
