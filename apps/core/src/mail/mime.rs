...
//! value rather than failing the whole receive (a dropped inbound is worse than a
...
/// site (`attachment-N`) when this returns None. Kept as a seam so a version-
/// specific extractor can be dropped in without touching the loop.
...
pub fn parse_raw(raw: &[u8]) -> Option<ParsedInbound> {
    ...
    for (idx, part) in msg.attachments().enumerate() {
        // mail-parser's per-part accessors vary across minor versions; derive the
        // filename from the Content-Disposition/Content-Type `filename`/`name`
        // attribute when present, else a stable fallback. Content-type is not
        // trusted for rendering anyway (the download route forces octet-stream).
