...
use super::retrieval::Embedder;
use super::spaces::open_vec_connection;
...
impl MessageIndex {
    /// Open (or create) the message index at the default path using the
    /// environment-configured model registry's embedder.
    pub fn open_default() -> Result<Self> {
        let registry = crate::registry::ModelRegistry::from_env();
        let embedder = Embedder::from_registry(&registry);
        Self::open_inner(default_db_path(), embedder)
    }

    fn open_inner(path: PathBuf, embedder: Embedder) -> Result<Self> {
    ...
    /// Open an in-memory index with the local (network-free) embedder at default
    /// registry dims. Used by tests.
    #[cfg(test)]
    ...
    /// Snapshot the embedder (cheap clone) so embedding I/O never holds a lock.
...
/// Mirrors the encoding in `spaces.rs`/`retrieval.rs`.
...
mod tests {
    ...
    async fn knn_round_trip_ranks_token_overlap_first() {
        let index = MessageIndex::open_in_memory().expect("open index");
