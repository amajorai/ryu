...
//! desktop Home grid), mirroring [`crate::quests::store`]. The refresh loop calls
...
use super::{Dashboard, DashboardEvent, DeviceDashboard, GridLayout, Widget};
...
fn default_db_path() -> PathBuf {
    crate::paths::ryu_dir().join("dashboards.db")
}
...
impl DashboardStore {
    /// Open (or create) the store at the default path (`~/.ryu/dashboards.db`).
...
mod tests {
    use super::*;
    use crate::dashboard::{WidgetKind, WidgetSource};
